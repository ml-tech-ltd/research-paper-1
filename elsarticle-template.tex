...
market model. %% Amaury: to describe the agents' beliefs? This may make sense,
%% but I'd need to elaborate a bit more.
...
% A trading strategy is a set of tools used by a trader in order to deter-mine
% when a particular type of trade should be executed. The type of a trade can be a
% buy or sell now order (which is executed immediately), or an entry order (which
% will be executed after the market price reaches a determined threshold). These
% orders can include other operations, like a stop loss or take profit orders (the
% trade will stop after a specific amount of units has been gained or lost), and
% trailing stops (a stop loss which gets constantly updated depending on the price
% movements). A trading strategy also incorporates a money management strategy,
% which determines the size of the lot to be traded.
...
membership function is equal to a random data point from the training dataset,
...
system is expressed by Equation~\ref{equation:ant-con-domain}. %% As can be seen in
%% the latter Equation, the domain of both the antecedents and consequents is
...
Subsection~\ref{subsection:meta-heuristic}.%% Talking about height.

% Amaury: Activations.

Indeterminacy in the agent rules is used by the agent to determine if
the inputs activate a rule or not, in conjunction with an activation
threshold---which is explained in the next paragraph. Indeterminacy is
used for this purpose in order to ``soften'' the activation
threshold. In other words, instead of having a hard threshold, where
the grade of membership has te be exactly greater than or equal to it
in order to activate a rule, the method uses indeterminacy to
transform 

As seen in Figure~\ref{figure:activation-threshold}, an
input is associated to a grade of membership---as is usual in a fuzzy
inference system---and this grade of membership can either belong or
not to an interval $\Lambda$ defined by
Equation~\ref{equation:activation-interval}. If the grade of
membership can be found in the interval

%% Fuzzy rules -> MF height -> Indeterminacy -> Activations
%% Fuzzy rules -> MF height -> 
...
%% Amaury: Role of indeterminacy.

%% The parts that are determined heuristically (what inputs and
%% outputs to use as MF means, greatest grade of membership in a
%% membership function) can be used to add a layer of interpretation.
